...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleRunAiAnalysis = async () => {
    ...
    try {
      ...
      setAiAnalysis("فشل التحليل بالذكاء الاصطناعي.");
    } finally {
  ...
  return (
    <div className={`min-h-screen bg-slate-50 flex ${lang === 'ar' ? 'rtl' : 'ltr'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      {/* Sidebar */}
      <aside className="w-20 md:w-64 bg-slate-900 text-white flex flex-col h-screen sticky top-0 shrink-0 z-50 shadow-xl">
        ...
        <div className="p-8 flex-1 overflow-y-auto">
            ...
            <div className="space-y-8 animate-in fade-in">
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    ...
                    <div key={idx} className="bg-white p-6 rounded-[2rem] border shadow-sm flex items-center gap-5">
               ...
               <div className="bg-gradient-to-br from-blue-700 to-indigo-800 rounded-[3rem] p-10 text-white shadow-xl relative overflow-hidden">
                  <div className="relative z-10 space-y-4">
                    <h3 className="text-xl font-bold flex items-center gap-3"><Sparkles className="text-amber-400" /> التحليلات الذكية (Gemini AI)</h3>
                    <p className="text-sm opacity-90 max-w-xl">احصل على ملخص شامل لأداء عمالك من خلال تحليل سجلات الحضور الميدانية والتقارير اليومية.</p>
                    <button onClick={handleRunAiAnalysis} className="bg-white text-blue-900 font-bold px-8 py-4 rounded-[1.2rem] shadow-xl text-xs flex items-center gap-2">
                       {isAnalyzing ? <Loader2 className="animate-spin" size={16} /> : <RefreshCw size={16} />} تحليل البيانات الميدانية
                    </button>
                    {aiAnalysis && <div className="mt-6 p-6 bg-black/20 backdrop-blur-md rounded-[2rem] text-sm leading-relaxed border border-white/10">{aiAnalysis}</div>}
                  </div>
               </div>
            </div>
          )}
               ...
               <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-8">
                    <div className="flex items-center gap-4">
                       ...
                       <div><h3 className="font-bold text-slate-800 text-xl tracking-tight">سجل التقارير الميدانية</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Field Operation Audit Logs</p></div>
                    </div>
                    <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-2xl border w-full md:w-auto overflow-x-auto shadow-inner">
                       ...
                       {departments.map(d => (
                         <button key={d.id} onClick={() => setReportFilterDept(d.id)} className={`px-6 py-2.5 rounded-xl text-[10px] font-bold uppercase whitespace-nowrap transition-all ${reportFilterDept === d.id ? 'bg-white shadow text-blue-600' : 'text-slate-400'}`}>{d.name}</button>
                       ))}
                    </div>
                  </div>
                          ...
                          <tr key={report.id} className="hover:bg-blue-50/20 transition-all group">
                            <td className="p-6">
                               <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-[10px] group-hover:bg-blue-600 group-hover:text-white transition-colors">{report.employeeName.charAt(0)}</div>
                            ...
                            <td className="p-6 text-[10px] font-bold text-slate-500 whitespace-nowrap">
                               ...
                               <span className="block opacity-40">{new Date(report.timestamp).toLocaleTimeString()}</span>
                            </td>
                            <td className="p-6">
                               <span className="text-[9px] px-3 py-1 bg-slate-100 rounded-full font-bold text-slate-500 uppercase tracking-wider">
                                  {departments.find(d => d.id === report.departmentId)?.name || 'عام'}
                               </span>
                            </td>
                            <td className="p-6 max-w-[300px]">
                               <p className="text-[11px] text-slate-600 leading-relaxed truncate">{report.content}</p>
                            </td>
                            <td className="p-6">
                               <div className="flex justify-center gap-2">
                                  ...
                                  {!report.attachmentUrl && <span className="text-[9px] text-slate-300">N/A</span>}
                               </div>
                            </td>
                          </tr>
                        ))}
          ...
          {activeTab === 'chat' && (
             <div className="flex flex-col h-[75vh] bg-white rounded-[3.5rem] border shadow-2xl overflow-hidden animate-in slide-in-from-bottom-8">
                <div className="p-6 bg-slate-50 border-b flex gap-3 overflow-x-auto shadow-inner">
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50/10">
                   {chatMessages.filter(m => chatTarget.type === 'all' || (chatTarget.type === 'dept' && m.departmentId === chatTarget.id)).map(msg => (
                     <div key={msg.id} className={`flex flex-col ${msg.senderId === 'ADMIN' ? 'items-end' : 'items-start'}`}>
                       <div className={`max-w-[75%] p-5 rounded-[1.8rem] text-[11px] shadow-sm transition-all hover:scale-[1.01] ${msg.senderId === 'ADMIN' ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none border border-slate-100'}`}>
                ...
                <div className="p-6 border-t bg-white flex gap-4 items-center">
                   <input 
                      ...
                      placeholder="اكتب رسالة توجيهية للقسم المختص..." 
                   />
                   <button onClick={handleSendChat} className="p-5 bg-blue-600 text-white rounded-[1.5rem] shadow-2xl hover:bg-blue-500 hover:scale-105 transition-all"><Send size={24} /></button>
                </div>
             </div>
          )}
          ...
          {activeTab === 'employees' && (
            <div className="space-y-8 animate-in slide-in-from-bottom-4">
               <div className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] border shadow-sm">
                  ...
                  <button onClick={() => setShowAddEmployeeModal(true)} className="bg-blue-600 text-white font-bold px-8 py-4 rounded-[1.5rem] shadow-xl hover:bg-blue-500 transition-all flex items-center gap-3 text-xs"><Plus size={20} /> إضافة عامل جديد</button>
               </div>
                  ...
                  {employees.filter(e => e.name.includes(searchQuery)).map(emp => (
                    <div key={emp.id} className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col items-center text-center group relative hover:border-blue-400 transition-all overflow-hidden">
                       ...
                       <div className="w-full space-y-3 pt-6 border-t border-slate-100">
                          <div className="flex justify-between items-center text-[10px] font-bold"><span className="text-slate-400">القسم:</span><span className="text-slate-800 font-bold">{departments.find(d => d.id === emp.departmentId)?.name}</span></div>
                          <div className="flex justify-between items-center text-[10px] font-bold"><span className="text-slate-400">الموقع:</span><span className="text-blue-700 truncate max-w-[120px]">{emp.workplace || 'غير محدد'}</span></div>
                       </div>
                    </div>
                  ))}
               </div>
      ...
      {(showEmployeeEditModal || showAddEmployeeModal) && (
        <div className="fixed inset-0 z-[100] bg-slate-900/70 backdrop-blur-md flex items-center justify-center p-6">
           <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
              ...
              <form onSubmit={handleSaveEmployee} className="p-10 space-y-6 max-h-[70vh] overflow-y-auto">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {editingEmp ? (
                      <>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-400 uppercase">الاسم الكامل</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-400 uppercase">رقم الهاتف</label>
                        ...
                        <div className="space-y-2 col-span-2">
                          <label className="text-[10px] font-bold text-slate-400 uppercase">الموقع الميداني (اسم المكان)</label>
                          <input value={editingEmp.workplace || ''} onChange={e => setEditingEmp({...editingEmp, workplace: e.target.value})} className="w-full bg-slate-50 border rounded-2xl p-4 text-sm font-bold shadow-inner" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-400 uppercase">Lat (خط عرض)</label>
                        ...
                        <div className="space-y-2">
                          <label className="text-[10px] font-bold text-slate-400 uppercase">Lng (خط طول)</label>
                 ...
                 <button type="submit" className="w-full bg-blue-600 text-white font-bold py-5 rounded-[1.5rem] shadow-xl hover:bg-blue-700 transition-all text-sm uppercase">تحديث البيانات الميدانية</button>
              </form>
           </div>
        </div>
      )}
      ...
      {selectedEmployee && (
        <div className="fixed inset-0 z-[110] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-6">
           <div className="bg-white w-full max-w-sm rounded-[3.5rem] shadow-2xl p-10 text-center space-y-6 animate-in zoom-in">
              ...
              <div className="space-y-4 text-right">
                 <div className="p-4 bg-slate-50 rounded-[1.5rem] border flex justify-between items-center"><span className="text-[10px] font-bold text-slate-400 uppercase">القسم:</span><span className="font-bold text-slate-800">{departments.find(d => d.id === selectedEmployee.departmentId)?.name || 'غير محدد'}</span></div>
                 <div className="p-4 bg-slate-50 rounded-[1.5rem] border flex justify-between items-center"><span className="text-[10px] font-bold text-slate-400 uppercase">الموقع:</span><span className="font-bold text-blue-700 truncate max-w-[150px]">{selectedEmployee.workplace || 'لم يحدد'}</span></div>
              </div>
              <button onClick={() => setSelectedEmployee(null)} className="w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-bold shadow-xl">إغلاق ملف الهوية</button>
           </div>
        </div>
      )}
    </div>
